...
class Agent:
    ...
    def _register_meta_tool(self):
        ...
        def create_and_use_tool(task_description: str, input_text: str = "") -> str:
            ...
            # Save to library — triple fallback for source code
            source = (
                getattr(new_tool, "_source_code", None)
                or getattr(agent_ref.skill_generator, "_last_code", None)
            )
            if agent_ref.skill_library and source:
                try:
                    agent_ref.skill_library.save(
                        name=new_tool.name,
                        code=source,
                        description=new_tool.description,
                        overwrite=True,
                    ...
                    if agent_ref.verbose:
                        print(f"  [SKILL] Saved: {new_tool.name}")
                except Exception as e:
                    print(f"  [SKILL] Save failed: {e}")
            elif agent_ref.skill_library and not source:
                print(f"  [SKILL] Warning: no source code attached to tool")
